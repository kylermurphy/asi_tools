function asi_ewkeo, $
  ...
  n_lon, $ ; number of latitude bins
  min_lon, $ ; minimum latitude
  max_lon, $ ; maximum latitude
  ...
  if site.length gt 1 then begin
    ...
    for i=0L, site.length-1 do begin
      ...
      if n_lon.length eq site.length then n_lon_p = n_lat[i] else n_lon_p=n_lat[0]
      ...
      ;get keo
      r_dat = asi_ewkeo(string(asi_loading), t0, dt, $
        n_lon_p,min_lon_p,max_lon_p, $
        min_lat_p,max_lat_p, $
  ...
  lon_axis = (keo.keo_coor.lon_low+keo.keo_coor.lon_up)/2.
  lon_range = [keo.keo_coor.lon_min,keo.keo_coor.lon_max]

  r_str = {asi_site:keo.asi_site, asi_array:keo.asi_array, asi_t:keo.asi_t, $
    asi_keogram:keo.asi_keogram, lon_axis:lon_axis, lon_range:lon_range}

  if keyword_set(add_tplot) then begin
    ;store the tplot data
    d = {x:r_str.asi_t, y:r_str.asi_keogram, v:r_str.lon_axis}
    data_att = {coord_sys:'Geomagnetic Longitude', units:'Counts/Intensity/Brightness'}
    dlimits = {SPEC:1,LOG:0, DATA_ATT:data_att, $
      YTITLE:r_str.asi_site+' '+r_str.asi_array +'!CEW Keogram', $
      ZTITLE:'Intensity', $
      ZLOG:1, X_NO_INTERP:1, Y_NO_INTERP:1, $
      OVERLAY:0}
    limits = {ZLOG:1, YLOG:0, YRANGE:lon_range, YSTYLE:1}
